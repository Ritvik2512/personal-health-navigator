...
# --- Neutral tool format ---
# tools.py defines tools like this:
# {
#   "name": "...",
#   "description": "...",
#   "parameters": {
...
class GeminiProvider:
    ...
    def format_tools(self, neutral_tools: List[Dict]) -> list:
        """Convert neutral tool format to Gemini protos."""
        declarations = []
        ...
        for part in candidate.content.parts:
            if hasattr(part, "function_call") and part.function_call.name:
                tool_calls.append({
                    ...
                    "id": part.function_call.name,  # Gemini has no id, use name
                })
    ...
    async def send_tool_results(self, results: List[Dict]) -> Dict:
        """Gemini tool results are handled inside the chat session in agent.py."""
        pass
...
class ClaudeProvider:
    ...
    def format_tools(self, neutral_tools: List[Dict]) -> list:
        """Convert neutral tool format to Claude input_schema format."""
        formatted = []
        for t in neutral_tools:
            props = {}
            for name, meta in t["parameters"].items():
    ...
    def format_history(self, history: List[Dict]) -> List[Dict]:
        """Convert neutral history to Claude format."""
        formatted = []
        for msg in history:
    ...
    async def chat(self, messages: List[Dict], system: str, tools: List[Dict]) -> Dict:
        ...
        return {
            ...
            "_raw_content": response.content,  # needed for Claude tool result loop
        }
